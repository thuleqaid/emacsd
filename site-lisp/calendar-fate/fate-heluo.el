...
(defun heluo_msg_output (year &optional lastguayao)
  (let* ((guayao (heluo_year_info year))
    ...
    (unless (and lastguayao (= (nth 2 lastguayao) (nth 2 guayao)))
      ...
      (setq tmpi (heluo_yuanqi_huagong)
            tmptext '())
      (insert (format pat2 "元气："))
      ;; 天元气
      (dolist (tmpj (list (mod (nth 0 guayao) 8)
                          (/ (nth 0 guayao) 8)
                          ))
        (when (memq tmpj (nth 0 tmpi))
          (add-to-list 'tmptext tmpj)
          (insert (format pat4 (format "%s（天元气）" (car (nth (* tmpj 9) gua-64))))))
        )
      ;; 地元气
      (dolist (tmpj (list (mod (nth 0 guayao) 8)
                          (/ (nth 0 guayao) 8)
                          ))
        (when (memq tmpj (nth 1 tmpi))
          (add-to-list 'tmptext tmpj)
          (insert (format pat4 (format "%s（地元气）" (car (nth (* tmpj 9) gua-64))))))
        )
      ;; 反天元气
      (dolist (tmpj (list (- 7 (mod (nth 0 guayao) 8))
                          (- 7 (/ (nth 0 guayao) 8))
                          ))
        (when (memq tmpj (nth 0 tmpi))
          (add-to-list 'tmptext (- 7 tmpj))
          (insert (format pat4 (format "%s（反天元气）" (car (nth (* (- 7 tmpj) 9) gua-64))))))
        )
      ;; 反地元气
      (dolist (tmpj (list (- 7 (mod (nth 0 guayao) 8))
                          (- 7 (/ (nth 0 guayao) 8))
        ...
        (when (memq tmpj (nth 1 tmpi))
          (add-to-list 'tmptext (- 7 tmpj))
          (insert (format pat4 (format "%s（反地元气）" (car (nth (* (- 7 tmpj) 9) gua-64))))))
        )
      (when (memq 0 tmptext)
        (insert (format pat4 "坤反，地道残，主贫")))
      (when (memq 1 tmptext)
        (insert (format pat4 "艮反，多痈气塞")))
      (when (memq 2 tmptext)
        (insert (format pat4 "坎反，多耳聋")))
      (when (memq 3 tmptext)
        (insert (format pat4 "巽反，多顽固疾病")))
      (when (memq 4 tmptext)
        (insert (format pat4 "震反，多跛足")))
      (when (memq 5 tmptext)
        (insert (format pat4 "离反，多目盲")))
      (when (memq 6 tmptext)
        (insert (format pat4 "兑反，唇齿多缺")))
      (when (memq 7 tmptext)
        (insert (format pat4 "乾反，天道失，主夭")))
      (insert (format pat2 "化工："))
      ;; 化工
      (dolist (tmpj (list (mod (nth 0 guayao) 8)
                          (/ (nth 0 guayao) 8)
                          ))
        (when (memq tmpj (nth 2 tmpi)) (insert (format pat4 (format "%s（化工）" (car (nth (* tmpj 9) gua-64))))))
        )
      ;; 反化工
      (dolist (tmpj (list (- 7 (mod (nth 0 guayao) 8))
                          (- 7 (/ (nth 0 guayao) 8))
                          ))
        (when (memq tmpj (nth 2 tmpi)) (insert (format pat4 (format "%s（反化工）" (car (nth (* (- 7 tmpj) 9) gua-64))))))
        )
      )
    ;; 输出大运批言
...
;; 计算当前用户对应的元气、化工
;; O: '(yuanqi_tian yuanqi_di huagong)
;; Memo: yuanqi_tian/yuanqi_di/huagong: '(gua ...)
;;       gua: 0~7
;; 反元气、反化工：7 - gua
(defun heluo_yuanqi_huagong ()
  (let* ((birthday (plist-get fate-user-current 'birthday))
         (jd (calendar-fate-chinese-datetime birthday))
         (sinfo (fate-solar-info birthday))
         (tinfo (calendar-fate-chinese-year (nth 2 birthday)))
         (yeargz (nth 1 sinfo))
         (tg (1+ (mod (1- yeargz) 10)))
         (dz (1+ (mod (1- yeargz) 12)))
         (termidx (nth 5 sinfo))
         (yuanqi_tian '())
         (yuanqi_di '())
         (huagong '())
    ...
    ;; 计算天元气
    (cond ((memq tg '(1 9)) (add-to-list 'yuanqi_tian 7 t))
          ((memq tg '(2 10)) (add-to-list 'yuanqi_tian 0 t))
          ((= tg 3) (add-to-list 'yuanqi_tian 1 t))
          ((= tg 4) (add-to-list 'yuanqi_tian 6 t))
          ((= tg 5) (add-to-list 'yuanqi_tian 2 t))
          ((= tg 6) (add-to-list 'yuanqi_tian 5 t))
          ((= tg 7) (add-to-list 'yuanqi_tian 4 t))
          ((= tg 8) (add-to-list 'yuanqi_tian 3 t))
          )
    ;; 计算地元气
    (cond ((memq dz '(11 12)) (add-to-list 'yuanqi_di 7 t))
          ((memq dz '(8 9)) (add-to-list 'yuanqi_di 0 t))
          ((memq dz '(2 3)) (add-to-list 'yuanqi_di 1 t))
          ((= dz 10) (add-to-list 'yuanqi_di 6 t))
          ((= dz 1) (add-to-list 'yuanqi_di 2 t))
          ((= dz 7) (add-to-list 'yuanqi_di 5 t))
          ((= dz 4) (add-to-list 'yuanqi_di 4 t))
          ((memq dz '(5 6)) (add-to-list 'yuanqi_di 3 t))
          )
    ;; 计算化工
    (cond ((and (< jd (cadr (nth 2 tinfo)))
                (>= jd (- (cadr (nth 2 tinfo)) 18)))    ;; 立春前18日
           (add-to-list 'huagong 0 t)
           (add-to-list 'huagong 1 t))
          ((and (< jd (cadr (nth 8 tinfo)))
                (>= jd (- (cadr (nth 8 tinfo)) 18)))    ;; 立夏前18日
           (add-to-list 'huagong 0 t)
           (add-to-list 'huagong 1 t))
          ((and (< jd (cadr (nth 14 tinfo)))
                (>= jd (- (cadr (nth 14 tinfo)) 18)))   ;; 立秋前18日
           (add-to-list 'huagong 0 t)
           (add-to-list 'huagong 1 t))
          ((and (< jd (cadr (nth 20 tinfo)))
                (>= jd (- (cadr (nth 20 tinfo)) 18)))   ;; 立冬前18日
           (add-to-list 'huagong 0 t)
           (add-to-list 'huagong 1 t))
          ((and (>= termidx 4) (< termidx 10))          ;; 春分--夏至
           (add-to-list 'huagong 4 t))
          ((and (>= termidx 10) (< termidx 16))         ;; 夏至--秋分
           (add-to-list 'huagong 5 t))
          ((and (>= termidx 16) (< termidx 22))         ;; 秋分--冬至
           (add-to-list 'huagong 6 t))
          (t (add-to-list 'huagong 2 t))                ;; 冬至--春分
          )
    (list yuanqi_tian yuanqi_di huagong)
    )
  )
