...
(setq thuleqaid/record-start-time '())
(defun thuleqaid/record-start ()
  (interactive)
  (let ((cmdfmt "ffmpeg -f dshow -i audio=\"マイク配列 (Realtek High Definition \" %s")
        (cmdcode 'japanese-shift-jis)
    ...
    (message (format cmdfmt thuleqaid/record-current-file))
    (let ((coding-system-for-read cmdcode)
          (coding-system-for-write cmdcode)
          )
      (async-shell-command (format cmdfmt thuleqaid/record-current-file))
      )
    )
  )
(defun thuleqaid/record-tag ()
  ...
  (let* ((curtime (current-time))
    ...
    (insert (format "Tag:%f[Sec]" diffsec))
    )
  )
